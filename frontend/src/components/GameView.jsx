...
export default function GameView({ mode, roomId, playerName, operatorId, onExit }) {
  ...
  // Request pointer lock from any click when not locked
  const handleDeployClick = useCallback(() => {
    if (!engineRef.current || isLocked || gameState.gameOver) return;
    // Request on the canvas container — the element the engine registered the listener on
  ...
  return (
    <div data-testid="game-view" className="relative w-screen h-screen overflow-hidden">
      {/* Game canvas — pointer lock is requested on this element */}
      <div
        ref={containerRef}
        id="game-canvas"
      ...
      {/* Deploy overlay — sits on top but forwards clicks to requestPointerLock */}
      {!isLocked && !gameState.gameOver && (
        <div
          className="absolute inset-0 flex items-center justify-center z-50 bg-black/80 backdrop-blur-sm cursor-pointer"
          ...
          <div className="text-center max-w-lg" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs font-mono text-[#4B5365] bg-black/40 border border-white/10 p-4 mb-6">
              ...
              <div className="col-span-2 pt-2 border-t border-white/5">
                <p><span className="text-[#00E5FF]">1/2/3</span> ou <span className="text-[#00E5FF]">SCROLL</span> - Trocar arma</p>
              </div>
            </div>
            {/* Big deploy button as fallback */}
            <button
              onClick={handleDeployClick}
              className="w-full bg-transparent border border-[#00E5FF] text-[#00E5FF] hover:bg-[#00E5FF] hover:text-[#050508] transition-all duration-200 uppercase tracking-widest px-8 py-3 mb-3 font-['Rajdhani'] font-bold text-lg shadow-[0_0_20px_rgba(0,229,255,0.3)] hover:shadow-[0_0_30px_rgba(0,229,255,0.7)]"
            >
              ▶ DEPLOY
            </button>
            <button
