...
export default function GameView({ mode, roomId, playerName, operatorId, settings, onExit }) {
  ...
  const [gameState, setGameState] = useState({
    ...
    score: { humans: 0, aliens: 0 }, isCrouching: false,
    isADS: false, isSprinting: false, leaning: 0,
    abilityCharge: 100, abilityActive: false, operatorId,
    damageIndicators: []
  });

  const handleStateUpdate = useCallback((update) => {
    if (update.locked !== undefined) setIsLocked(update.locked);
  ...
  useEffect(() => {
    ...
    engineRef.current = new GameEngine(containerRef.current, handleStateUpdate, { mode, operator, settings });
  ...
  const connectWebSocket = () => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === 'game_state' && data.game_over)
        setGameState(prev => ({ ...prev, gameOver: true, winner: data.winner }));
    };
    ws.onerror = () => {};
  };

  // ── KEY FIX: clicking the overlay itself triggers pointer lock ──
  ...
  return (
    <div data-testid="game-view" className="relative w-screen h-screen overflow-hidden">
      <div ref={containerRef} id="game-canvas" className="absolute inset-0" data-testid="game-canvas-container" />

      {/* ── Click-to-lock overlay — the whole div is clickable ── */}
      {!isLocked && !gameState.gameOver && (
        <div
          className="absolute inset-0 flex items-center justify-center z-50 bg-black/80 backdrop-blur-sm cursor-crosshair"
          onClick={handleOverlayClick}
        >
          <div className="text-center max-w-lg">
            <div className="mb-6 relative">
              <div className="w-16 h-16 mx-auto border-2 border-[#00E5FF] rounded-full flex items-center justify-center animate-pulse">
            ...
            <p className="text-sm font-mono text-[#8B93A6] mb-6">
              Click to lock cursor and begin combat
            </p>
            <div className="grid grid-cols-2 gap-4 text-xs font-mono text-[#4B5365] bg-black/40 border border-white/10 p-4 mb-6">
              ...
              <div className="col-span-2 pt-2 border-t border-white/5 text-center">
                <p><span className="text-[#00E5FF]">1/2/3</span> or <span className="text-[#00E5FF]">SCROLL</span> — Switch weapon &nbsp;|&nbsp; <span className="text-[#00E5FF]">ESC</span> — Unlock cursor</p>
              </div>
            </div>
            <button
              onClick={(e) => { e.stopPropagation(); handleMenu(); }}
              className="bg-transparent border border-white/20 text-white hover:border-white hover:bg-white/10 transition-all duration-200 uppercase tracking-widest px-6 py-2 text-xs font-mono"
            >
              BACK TO MENU
            </button>
          </div>
